...
- обновляет индексы изменяемых источников до построения очередей и сохраняет
  дельту единиц (`added`, `changed`, `removed`, `unchanged`, технические и
  отклонённые как нерелевантные);
- вызывает только явно зарегистрированные адаптеры, а агентный маршрут
  принимает только через явное подтверждение ручного запуска;
- для адаптера версии 2 выполняет `probe` перед `index`, `fetch` или `verify`, не
...
- различает исполнение, управляемую паузу, ожидание решения, ошибку и
  завершение;
- закрывает глобальные стадии понятий, влияния, переноса изменений и проверки, в
  том числе явной командой агента, если для стадии не зарегистрирован
  проектный исполнитель;
- строит очередь `coverage_gap` из карты охвата (`source-map.yml`) каждого
  длинного источника, не давая проходу завершиться при незакрытой структурной
  единице;
- повторяет операционную проверку перед завершением.
- собирает обязательную сводку владельца из дельты индексов, утверждений,
  отчёта влияния, политики переноса и блокеров.
...
[ADR-0007](ADR/ADR-0007.md). Ручное закрытие глобальной стадии без
зарегистрированного исполнителя определяет [ADR-0008](ADR/ADR-0008.md).
Включение карты охвата длинного источника в критерий завершения прохода
определяет [ADR-0009](ADR/ADR-0009.md).

Договор обновления индексов, агентного маршрута, политики переноса и сводки
фиксирует [ADR-0010](ADR/ADR-0010.md).
...
только локальное наблюдение доступности. `index` получает навигационный индекс
без массового содержательного получения. `fetch` получает конкретный снимок,
...
`probe`, `index`, `fetch`, `verify` и необязательную `authorize`. Неизвестная версия или
имя адаптера отклоняются. Секреты не передаются в аргументах, стандартном
выводе, отчёте или отслеживаемых настройках.

`adapter: agent` не является записью реестра: он задаёт проверяемый ручной
маршрут получения самим агентом. Старое `adapter: manual` читается только в
режиме совместимости и не считается воспроизводимым маршрутом без
`agent_route`.
...
Перед первым исполняемым шагом источник проходит обновление индекса и сравнение
с предыдущим успешно сохранённым снимком. После каждого исполняемого шага
очередь должна наблюдаемо измениться.
...
После `impact_audit` режим `apply_now` передаёт разрешённые выводы стадии
`apply_changes`. Режим `propose_only` закрывает эту стадию политикой, сохраняет
проверяемые предложения в локальном отчёте и запрещает изменения вне корпуса и
`.local`. Анализ влияния охватывает требования, решения, навыки и правила, код,
конфигурацию, данные, тесты и модельные сценарии, сборку, поставку и
пользовательскую документацию; область записи остаётся уже и определяется
проектом. `apply_now` закрывается только после доказанного изменения ожидаемого
артефакта либо совпадения его проверенного хэша до запуска.
...
| ПТ-003, ПТ-004, ФТ-011, ФТ-017, ФТ-018, КТ-003 | оркестратор и долговечное состояние | ADR-0001, ADR-0010, тесты операций и контроллера прохода |
...
| ФТ-010, ФТ-012, ФТ-018, ОГР-003 | анализ влияния и проверка, отделённые от исправления | сценарии `kc-impact-audit`, `kc-pipeline` и `kc-validation` |
...
| Изменение обновления индекса или маршрута получения | `kc-inventory`, `kc-pipeline` и карточка источника | ADR-0010, миграция `manual`, дельта и тесты адаптеров |
| Изменение политики переноса или итоговой сводки | `kc-pipeline`, `kc-impact-audit`, `kc-status` | ADR-0010, отчёт влияния и сценарии `propose_only` |
