...
## Ручной полный проход

Ручной запуск `kc-pipeline` сначала обновляет индексы зарегистрированных
изменяемых источников, сохраняет дельту и только затем строит очереди. В дельте
различаются новые, изменённые, снятые с публикации, неизменившиеся,
нерелевантные и только технически изменившиеся единицы. Для `index_only` и
`external_reference` это не означает массового получения материала: дальше
проходят только единицы, отобранные проектной политикой.

Политика переноса выбирается явно: `apply_now` оставляет безопасное применение
в том же проходе, а `propose_only` обновляет корпус и выполняет анализ влияния,
но не меняет производные файлы продукта. Все предложения и отдельные вопросы
для владельца попадают в обязательную сводку вместе с источником, основанием,
поверхностями влияния, ограничениями и следующим действием.

Для агентного обновления индекса одного флага недостаточно: агент передаёт
локальное свидетельство с `run_id`, хэшами до и после, проверенными и изменёнными
путями, разрешённой областью записи и связью с источником.
